...
body.classList.toggle('dark-theme', isDarkTheme); - This line toggles the dark-theme class of the body element. 
The classList property is a read-only list that contains the classes of an element. 
The toggle() method adds a class to the element if it does not have it, or removes it if it does. 

The toggle() method accepts an optional second argument,
- Behavior: The second argument is a "force" boolean.
- If true: It must add the class (even if it's already there).
- If false: It must remove the class (even if it's already gone).
- The Benefit: This ensures that the DOM always matches your state. It’s a "source of truth" approach rather than just a "flip-flop" approach.
